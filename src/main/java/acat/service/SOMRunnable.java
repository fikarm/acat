 ...
 *  Haykin (1997)
 */
public class SOMRunnable implements Runnable {

    private double[][]         input;         // row = data, column = feature
    private double[][][]       weight;        // neuron's weight
    private int                width, height, // lattice width and height
                               inputSize,     // input vector size
                               iteration;
    private double             radius,        // σ0
                               learningRate,  // η0
                               time1,         // τ1 = Eq.(9.6)  time constant
                               time2;         // τ2 = Eq.(9.14) time constant
    ...
    // queues ui update
    private final BlockingQueue<Map<String, int[]>> tagQueue;
    private final BlockingQueue<double[][]> umatrixQueue;
    private final BlockingQueue<String> messageQueue;

    // processing state
    public boolean isFinished = false;

    public SOMRunnable(double[][] input, String[] tags, int mapSize,
                       BlockingQueue<double[][]> umatrixQueue,
                       BlockingQueue<Map<String, int[]>> tagQueue,
        ...
        width        = mapSize;
        height       = mapSize;
        iteration    = 500 * width * height;
        radius       = width / 2.0;
        learningRate = 0.1;
        time1        = iteration / log(radius);
        time2        = iteration;
        random       = 1;
        ...
        // initialize BlockingQueue instances
        this.umatrixQueue = umatrixQueue;
    ...
    public void run() {
        ...
        try {
            ...
            double   sigma,               // decaying radius
                     eta = learningRate;  // decaying learning rate, never below 0.01

            for ( int n = 0; ( n < iteration ) && ( eta >= 0.01 ); n++ ) {

                r     = pickRandomSample();
                x     = input[r];
                i     = winningNeuron( x );
                sigma = decayRadius( n );
                eta   = decayLearningRate( n );

                updateWeight( x, i, sigma, eta );

                tagBMU.replace( tags[r], i );

                // put a change to queue every 250 iterations
                if ( n % 250 == 0 ) {

                    tagQueue.put(tagBMU);
                    umatrixQueue.put( getUMatrix() );
                    messageQueue.put( String.format( "SOM training iteration %,d / %,d", iteration, n ) );
