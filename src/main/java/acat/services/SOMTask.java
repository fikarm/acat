...
public class SOMTask extends Task<double[][]> {

    private final double[][]         input;         // row = data, column = feature
    private double[][][]       weight;        // neuron's weight
    private final int                width;
    private final int                height;        // lattice width and height
    private final int                inputSize;     // input vector size
    private final double             radius;        // σ0
    private final double             learningRate;  // η0
    private final double             time1;         // τ1 = Eq.(9.6)  time constant
    private final double             time2;         // τ2 = Eq.(9.14) time constant
    ...
    private final Map<String, int[]> tagBMU;      // holds winning neurons tag
    private final int                maxIter;
    private final IntegerProperty    intervalProperty;

    public SOMTask( double[][] input, String[] tags, int mapSize ) {
        ...
        intervalProperty = new SimpleIntegerProperty( 0 );
    ...
    protected double[][] call() {
        ...
        double interval = ( maxIter / ( maxIter / 600.0 ) );
        String maxIterFormat = formatter.format( maxIter );


        for ( int iter =  0 ; ( iter < maxIter) && ( eta >= 0.01 ); iter++ ) {
            ...
            x     = input[r];
            i     = winningNeuron( x );
            sigma = decayRadius(iter);
            eta   = decayLearningRate(iter);
            ...
            if ( iter % interval == 0 ) {
                ...
                intervalProperty.set( iter );
                updateMessage( "Training SOM iteration " + maxIterFormat + "/" + formatter.format(iter) );
                updateProgress( iter, maxIter );
                
            }

        }

        intervalProperty.set( maxIter );
