...
public class Doc2Vec extends Word2Vec{
    ...
    private final String  doc_vec_output_file;
    private final String output;
    private final SimpleBooleanProperty isCancelled = new SimpleBooleanProperty();

    public Doc2Vec( 
        String input, String pvOutput, 
        int layerSize, int window, int iterations,
        float alpha, float subsample, int negativeSample, 
        boolean useHS 
    ){
        ...
        output              = pvOutput;
        train_file          = input;
        save_vocab_file     = output + "/pv-vocab.txt";
        output_file         = output + "/wv-vector.txt";
        doc_vec_output_file = output + "/pv-vector.txt";
    ...
    void InitNet() {
        ...
        //save initial weight
        saveVector(syn0, (int)vocab_size, (int)layer1_size, "syn0.txt");
        saveVector(syn0doc, doc_size, (int)layer1_size, "syn0doc.txt");
    ...
    void TrainModelThread() {
        ...
        long word;
        long last_word;
        long sentence_length = 0;
        long sentence_position = 0;
        long word_count = 0;
        long last_word_count = 0;
        ...
        float f;
        float g;
        float[] neu1 = new float[(int)layer1_size];
        float[] neu1e = new float[(int)layer1_size];
            ...
            while( !isCancelled.get() ){
                ...
                if (word_count - last_word_count > 10_000) {
                    ...
                    if (debug_mode > 0) {
                        ...
                        System.out.printf(
                                "Alpha: %f  Progress: %.2f%%  \r", alpha,
                                word_count_actual / (float) (iter * train_words + 1) * 100
                        );
//                        updateMessage( String.format(
//                                "training doc2Vec, alpha: %f  Progress: %.2f%%",
//                                alpha,
//                                word_count_actual / (float) (iter * train_words + 1) * 100)
//                        );
                    ...
                    if (cw > 0) {
                        ...
                        if (negative > 0) {
                            for (d = 0; d < negative + 1; d++) {
                                ...
                                else if (f < -MAX_EXP) g = (label) * alpha;
                    ...
                    for (a = b; a < window * 2 + 1 - b; a++) {
                        if (a != window) {
                            ...
                            if (negative > 0) for (d = 0; d < negative + 1; d++) {
                                ...
                                for (c = 0; c < layer1_size; c++)
                                    syn1neg[(int) (c + l2)] += g * syn0[(int) (c + l1)];
                            }

                            for (c = 0; c < layer1_size; c++) syn0[(int) (c + l1)] += neu1e[(int) (c)];
                        }
                    }
                }
...
//            updateMessage( String.format( "Start training using file %s\n", train_file ) );
    ...
    private void saveVector(float[] vector, int rows, int cols, String name) {
